  ...
  for (const name of Object.keys(webhooks)) {
    ...
    var checkOrigin = new Promise(function(resolve, reject) {
      ...
      try {
        ...
        console.log('origin:', _.result(origin, 'push.changes[0].new.name', ''));
      ...
      pm2.describe(name, function(err, apps) {
        console.log('req:', apps[0].pm2_env.versioning.branch);
...
function pullAndReload(name) {
  ...
  async.series([
    ...
    function(callback) {
      ...
      vizion.update({
        folder: cwd
      }, logCallback(cb, '[%s] Successfuly pulled application %s', new Date().toISOString(), name));
    },
    // Pre-hook
    function(callback) {
      if (!current_app.pre_hook) return callback(null);

      spawnAsExec(current_app.pre_hook, execOptions,
        logCallback(cb, '[%s] Prehook command has been successfuly executed for app %s', new Date().toISOString(), name));

    },

    // Pull and restart
    function(callback) {
      pm2.gracefulReload(name,
        logCallback(cb, '[%s] Successfuly reloaded application %s', new Date().toISOString(), name));
    },
